...
export function PermissionsProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadPermissionsFallback = async () => {
    try {
      ...
      const isUserAdmin = profileData.roles?.name === 'admin'
      ...
      if (isUserAdmin) {
        ...
        submenusData = (allSubmenus || []).map(submenu => ({
          id: submenu.id,
          name: submenu.name,
          label: submenu.label,
          route: submenu.route,
      ...
      setUserPermissions({
        ...
        role: {
          ...
          name: profileData.roles?.name || 'sin_rol',
          description: profileData.roles?.description || 'Sin descripción'
        },
        menus: menusData,
        submenus: submenusData
      })

      console.log('✅ Modo fallback activado - Rol:', profileData.roles?.name)
