...
function getWeekDays() {
  ...
  for (let i = 0; i <= diff; i++) {
    ...
    const start = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), 3, 0, 0));
    const isToday = i === diff;
    const end = isToday ? new Date() : new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate() + 1, 2, 59, 59));
  ...
  for (const day of days) {
    ...
    for (const companyId of companies) {
      ...
      for (const driver of drivers) {
        try {
          // CORREGIDO: Agregados rejected, dropOffs, connected al query
