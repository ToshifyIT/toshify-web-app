// Edge Function: Sincronización Semanal Automática de Cabify
// Se ejecuta vía cron job cada lunes a las 2 AM
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Calcular última semana completa
    const { startDate, endDate, label } = getLastWeekRange()

    console.log(`📅 Sincronizando semana: ${label}`)
    ...
    if (existing && existing.length > 0) {
      console.log('⏭️  Período ya sincronizado, saltando...')

      return new Response(JSON.stringify({
        status: 'skipped',
        message: `Period ${label} already synced`,
